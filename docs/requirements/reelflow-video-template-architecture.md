...
### 7.2.1 代码优先（high-code）—— 修正"无内嵌代码"红线
- **模板的源 = 仓库 TS 代码**（最高可读性 / 扩展性 / 灵活性，支撑各类视频模板）。两种风格都产出 Resolved IR：
  1. **声明式 recipe**（`defineRecipe(config)`）：常见结构走配置；
  2. **命令式 director**（`defineTemplate({ run(ctx,input) })`）：新颖/复杂视频用全代码，任意逻辑。
- "无内嵌代码"红线**只约束 DB 存储的发布快照**（运行期/表单元数据），**源始终是代码**；DB 存其序列化产物。
- **画布承接高代码**：代码是一等节点 `[模板代码] → [脚本] → [分镜卡] → [草稿/MP4]`，代码与预览同屏对照、双向联动（改代码→重新 preview→输出刷新），即"live-coding 笔记本画布"。React Flow 自定义节点同时承载 视频/图/JSON/**可编辑代码**。
