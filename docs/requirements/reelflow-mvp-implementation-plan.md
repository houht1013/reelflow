...
| 2.4 | **视频生成工具** | ⏸️ 暂缓。当前未接入任何文生视频/图生视频 provider，无法落地"补能力"；侧栏保持"准备中"。待拿到视频 provider 再排期。 |
| 2.5 | **测试覆盖补齐** | 🟡 进行中（2026-06-27）。已加纯逻辑单测：`resolveWorkspaceImageConcurrency`(钳1-5)、熔断器(打开/重置/快速失败)、`ossThumb`(共 23 例全过)。待补：模板执行/断点恢复/结算边界（需 DB，作为集成测试后续补）。|
...
| 3.2 | **真实支付接入** | ⏳ 待办（阻塞）。**优先接入支付宝**（手机网站/扫码支付）；当前 checkout 为 RESERVED 模拟。**前置条件：支付宝商户审核通过**——审核通过后再开发：对接支付宝支付 + 异步回调，复用现有 `dev-complete-purchase` / `grantWorkspaceSubscriptionCredits` 的发放逻辑改为真实回调驱动。微信/银行卡/PayPal 暂不接入。 |
